...
## Correcciones posteriores — el codigo ya no es identico a este plan

Lo que sigue se arreglo **despues** de ejecutar la tarea, y el codigo en la rama
manda sobre los bloques de arriba. Se anota aqui para que nadie transcriba dos
veces un defecto ya pagado.

| Commit | Que estaba mal |
|---|---|
| `044579f` | El test de `EDGAR_IDENTITY` pasaba con guarda y sin ella: el doble lanzaba `AssertionError` con un mensaje descriptivo y la asercion por subcadena se satisfacia con ese mismo mensaje |
| `35ab65a` | **`adjunto.text()` devuelve `None` para lo que no es texto.** Un `EX-99` en pdf reventaba el `join` y tiraba el `EX-99` bueno que estaba al lado, reportandolo como fallo de la SEC. Reproducido con Axos Financial, `0001299709-26-000056`: 15.210 caracteres buenos descartados |
| `b71a232` + `cb75120` | `aplicar_tope` cortaba a mitad de palabra, y **el primer arreglo fue peor que el defecto**: retrocedia al ultimo salto de parrafo estuviera donde estuviera, y con el EX-99.1 de MSFT eso tiraba veinticinco mil caracteres para salvar una palabra. Ahora se retrocede dentro de `_RETROCESO` |
| `5d51ec6` | `_pct` redondeaba a entero: 4,3% y 4,4% se leian los dos como «4%», y un 0,4% como «0%» |
| `891d1ca` | La rama `viable=True` de `contexto.operaciones` no tenia test |

**Los cinco venian de este plan, no de quien lo ejecuto.** Y los cinco salieron de
mirar datos reales: uno de muestrear ~190 expedientes de EDGAR en vivo, otro del
test `red` que empezo a fallar. Ninguno se veia leyendo el codigo.
